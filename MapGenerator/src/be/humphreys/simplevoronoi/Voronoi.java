...
public class Voronoi
{
    ...
    public List<GraphEdge> generateVoronoi(List<Double> xValuesIn, List<Double> yValuesIn,
    		double minX, double maxX, double minY, double maxY) {
    	double[] xInputs = new double[xValuesIn.size()], yInputs = new double[yValuesIn.size()];
    	if(xInputs.length != yInputs.length) return null;
    	
    	for(int i = 0; i < xInputs.length; i++){
    		xInputs[i] = xValuesIn.get(i);
    		yInputs[i] = yValuesIn.get(i);
    	}
    	return generateVoronoi(xInputs, yInputs, minX, maxX, minY, maxY);
    }
